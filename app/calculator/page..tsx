...
export default function CalculatorPage() {
  const [courses, setCourses] = useState([{ id: 1, name: "", units: "", grade: "" }])

  const addCourse = () => {
    setCourses([...courses, { id: courses.length + 1, name: "", units: "", grade: "" }])
  }
    ...
    <div className="px-6 py-8">
      <div className="mb-6 flex items-center">
        <Link href="/dashboard" className="flex items-center gap-2">
          ...
          <span className="text-lg font-medium">CGPA Calculator</span>
        </Link>
      </div>
        ...
        {courses.map((course) => (
          <div key={course.id} className="rounded-lg bg-gray-100 p-4">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-sm font-medium">Course {course.id}</h3>
